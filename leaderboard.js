...
function fetchFirebaseLeaderboard(dbInstance, syncTimestamp) {
  dbInstance.ref("users").once("value").then((snapshot) => {
    ...
    leaderboardUsers = Object.keys(usersObj)
      .filter((uidKey) => !usersObj[uidKey].isBanned)
      .map((uidKey) => {
...
function updateUserRankCard(sortedList = []) {
  ...
  const listToSearch = sortedList.length > 0 ? sortedList : leaderboardUsers;
  if (!listToSearch.length) return;

  const rankIndex = listToSearch.findIndex((u) => u.uid === currentUserId);
  const userObj = listToSearch[rankIndex];
  ...
  if (rankIndex !== -1 && userObj) {
    ...
    if (subtextElem) subtextElem.innerText = `#${rankIndex + 1} of ${listToSearch.length} miners`;
